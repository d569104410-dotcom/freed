/* ============================================================
   FREED 2026 - MASTER SQL SCHEMA (FOLLOW-UP ONLY) - RELEASE HARDENED v3
   File         : FREED_MASTER_SCHEMA_FOLLOWUP_UNIFIED_INFERRED_RCFinal_ReleaseHardened_v3.sql
   Source       : ReleaseClean_v2
   Purpose      : Hardened deployment edition for controlled execution
   ------------------------------------------------------------
   HARDENING NOTES
   ------------------------------------------------------------
   - Uses guarded execution guidance (precheck + postcheck + QA queries)
   - Keeps optional/unknown-environment FKs commented where appropriate
   - Includes duplicate-constraint mitigation helper snippets (commented)
   - Intended for MySQL 8.0+ and controlled DBA execution
   ============================================================ */

-- Recommended session modes (review before enabling in legacy environments)
-- SET SESSION sql_mode = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
-- SET SESSION innodb_strict_mode = 1;


-- ============================================================
-- [HARDENING][OPTIONAL] RE-RUN HELPERS (COMMENTED)
-- Use only when rerunning after a partial/failed deployment that left objects behind.
-- These snippets are intentionally manual/reviewed (not auto-generated destructive logic).
-- ============================================================
/*
-- Example pattern to inspect existing constraints by table:
SELECT table_name, constraint_name, constraint_type
FROM information_schema.table_constraints
WHERE table_schema = DATABASE()
  AND table_name IN ('strategic_axes','strategic_goals','workflow_instances');

-- Example pattern to inspect indexes:
SHOW INDEX FROM strategic_goals;
SHOW INDEX FROM workflow_instances;

-- If a specific FK exists and blocks re-run, drop it explicitly:
-- ALTER TABLE workflow_instances DROP FOREIGN KEY fk_wi_current_step;

-- If a duplicate named index exists and blocks re-run, drop it explicitly:
-- ALTER TABLE strategic_goals DROP INDEX uk_sg_org_code;
*/
...
-- ============================================================
-- [10] HARDENED v3 - EXTRA QA SMOKE CHECKS
-- ============================================================

-- Count created extension/follow-up tables
SELECT COUNT(*) AS created_extension_tables
...
-- Verify FK count for major modules (informational)
SELECT rc.constraint_name, rc.table_name, rc.referenced_table_name
FROM information_schema.referential_constraints rc
WHERE rc.constraint_schema = DATABASE()
  AND rc.table_name IN (
    'strategic_goals','operational_goals','kpi_links','risk_mitigations',
    'donations','collections','initiative_stock_requests','workflow_instances','workflow_instance_steps'
  )
ORDER BY rc.table_name, rc.constraint_name;

-- Find nullable links with mismatched entity type in kpi_links (should be reviewed by app validations too)
SELECT id, link_entity_type, initiative_id, strategic_goal_id, operational_goal_id, department_id, task_id
FROM kpi_links
WHERE
...
-- Find workflow instances with current_step_id not belonging to same instance (should return 0 rows)
